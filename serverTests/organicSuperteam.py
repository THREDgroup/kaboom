...
subteamsReverse = [subteamsSortedByStyle[-(i+1)] for i,s in enumerate(subteamsSortedByStyle)]
...
organicSTscores = [t.getBestScore() for t in organicSuperTeamObjects]
#plt.scatter([1 for i in organicSTscores],organicSTscores)
##plt.scatter([1],)
#plt.scatter([0 for i in controlScores],controlScores)


box=plt.boxplot([np.array(controlScores)*-1, np.array(organicSTscores)*-1],labels= ["organic","organic worst allocation"],showfliers=True)

plt.savefig("/Users/samlapp/SAE_ABM/figs/carBadTeamAssignment.pdf")

#plt.xticks()
#h.pScore(allHomogeneousScores,superTeamScores2)
print("effect size:" )
print(h.effectSize(controlScores,organicSTscores))
